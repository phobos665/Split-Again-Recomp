...
/* Relative to the executable's own directory, build/<Config>/. The game's own
 * files are not shipped: put them in a folder called `game` at the root of
 * this repository, so that game\default.xbe exists. A `game` folder beside
 * the executable and RECOMP_GAME_DIR both take priority -- see find_game. */
